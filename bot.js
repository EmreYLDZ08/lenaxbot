...
client.on("message", msg => {
  var dm = client.channels.get("754020413683662868"); //mesajın geleceği kanal idsi//
...
client.on("message", m => {
  if (m.channel.id !== "738138011371307061") { //buraya o kanalın ID'si yazılacak.
    return;
  }
...
.setColor('RANDOM')
.setTitle('Hoşgeldin')
.setDescription('***Roleplay Sunucumuza Hoşgeldin Kanki***  https://media.giphy.com/media/H3TIAPUjmCKNDzAGpm/giphy.gif                                    https://media.giphy.com/media/WoFo0SgRrluSTORxoX/giphy.gif')
member.send(embed)
})
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'merhaba') {
		if (!msg.guild.member(msg.author).hasPermission("ADMINISTRATOR")) {
			msg.author.sendMessage('**Merhaba reyiz**'); 
		} else {
		msg.reply('Merhaba kanki');
		}
	}
});
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'bot naber') {
		if (!msg.guild.member(msg.author).hasPermission("ADMINISTRATOR")) {
			msg.author.sendMessage('**İyidir reyiz senden naber ?**'); 
		} else {
		msg.reply('İyidir kanki senden naber ?');
		}
	}
});
...
client.on("guildMemberAdd", async member => {
  ...
  channel.setNickname("Son Üyemiz: " + member.user.username);
});
...
client.on("ready", async () => {
  ...
  let embed = new Discord.RichEmbed()
    ...
    .setColor("40bcdb")
    .setFooter(`${client.user.username} `, client.user.avatarURL);
  client.channels.get("754391164072886287").send(embed); //mesaj göndereceği kanal
});
